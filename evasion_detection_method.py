...
def count_evasion(
    ...
    def getStationPolygon(frame):
        ...
        results_station = station_model.predict(img, conf=0.90, verbose=False)
    ...
    if(first_frame_polygon is not None and last_frame_polygon is not None):
        ...
        # print('usando el polígono del primer frame' if first_frame_ratio < last_frame_ratio else 'usando el polígono del último frame')
    else:
    ...
    # Diccionarios
    trajectories = defaultdict(list) #{id: [(x,y), (x,y), ...]}
    track_colors = {} #color único por track
    ...
    # -------------------  sacar distancia de un punto al polígono de la estacoin -------------------
    ...
    for result in tracker:
            ...
            for (x1, y1, x2, y2, conf, track_id) in zip(boxes[:,0], boxes[:,1], boxes[:,2], boxes[:,3], confs, ids):
                ...
                if track_id not in previous_positions:
                    ...
                    if inside:
                        ...
                        # print(f"ID {track_id} ignorado - apareció dentro de la estación")
                ...
                if not was_inside and inside and track_id not in entered_ids:
                    # Verificar que el track estuvo fuera por más de 2 segundos
        ...
        for track_id in disappeared_ids:
            ...
            if track_id in last_seen_position and track_id in first_seen_frame and station_polygon is not None:
                # Verificar que la trayectoria existió por más de 2 segundos
                ...
                if total_time >= MIN_TIME_OUTSIDE:
                    ...
                    # Verificar que el último punto está cerca del polígono (300px)
        ...
        for track_id, (ex, ey, tipo) in evasion_marks.items():
            ...
            # Círculo de marca
            cv2.circle(annotated_frame, (ex, ey), 15, color, -1)
            cv2.circle(annotated_frame, (ex, ey), 17, (0, 0, 255), 3)
            
            # Texto
            text = f"ID {track_id} - Evasion"
        ...
        count_text = f"Entradas: {len(entered_ids)} | Evasiones: {len(evasion_detected)}"
        ...
        if not headless:
            cv2.imshow("Evasion Detection", annotated_frame)
    ...
    # print(f"\n--- RESUMEN ---")
    # print(f"Total de tracks que entraron al polígono: {len(entered_ids)}")
    # print(f"Total de tracks ignorados (aparecieron dentro): {len(ignored_ids)}")
    print(f"\n      Total de evasiones detectadas: {len(evasion_detected)}")
    # print(f"  - Caso 1 (permanencia >2s): {sum(1 for t in evasion_marks.values() if t[2] == 'entrada')}")
    # print(f"  - Caso 2 (desaparición cerca): {sum(1 for t in evasion_marks.values() if t[2] == 'desaparición')}")
